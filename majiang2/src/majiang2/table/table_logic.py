                        ...
                        for rsi in range(len(roundStat)):
                            for statItems in roundStat[rsi]:
                                for oneStatItemKey in statItems.keys():
                                    if oneStatItemKey == MOneResult.STAT_DIANPAO:
                                        dianpaoName = self.roundResult.roundResults[ri].statType[MOneResult.STAT_DIANPAO]["name"]
                                        dianpaoFanPattern = [dianpaoName, str(0)+"番"]
                                        if dianpaoFanPattern not in fanPattern[rsi]:
                                            fanPattern[rsi].append(dianpaoFanPattern)
                    ...
                    for rsi in range(len(roundStat)):
                        for statItems in roundStat[rsi]:
                            for oneStatItemKey in statItems.keys():
                                ...
                                if MPlayMode().isSubPlayMode(self.playMode, MPlayMode.QUESHOU) and oneStatItemKey == MOneResult.STAT_ANGANG:
