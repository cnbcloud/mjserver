...
from luosihu.table.majiang_friend_table import LuosihuMajiangFriendTable
from luosihu.table.majiang_quick_table import LuosihuMajiangQuickTable
from luosihu.entity.configure.conf import GAMEID
from poker.entity.dao import gamedata

class TGLuosihu(TYGame):

    def __init__(self, *args):
        super(TGLuosihu, self).__init__()
        self._lastSendIntervalLedTime = int(time.time()) #上一次下发'每隔多久LED'LED的时间
        
    def initGameBefore(self):
        from luosihu.entity import task_inspectors
    ...
    def newTable(self, room, tableId):
        ...
        tableId 新桌子实例的ID
        '''
        playMode = room.roomConf.get("playMode", "luosihu")
        ...
        if isCreateTable:    
            table = LuosihuMajiangFriendTable(tableId, room)
        else:
            table = LuosihuMajiangQuickTable(tableId, room)
        return table
