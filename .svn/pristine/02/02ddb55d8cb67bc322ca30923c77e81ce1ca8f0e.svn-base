...
特点：
1）摸牌胡牌
2）摸到的牌手里有碰或者暗刻，可在不改变听口的情况下组成杠牌
相当于多了一张宝牌

"""
class MDafengProcessor(MProcessor):
    def __init__(self):
        super(MDafengProcessor, self).__init__()
    ...
    def getState(self):
        if self.state != MTableState.TABLE_STATE_NEXT:
            ftlog.debug('MLouHuProcessor.getState return:', self.state)
        return self.__state
